...
class Labirinto:
    ...
    def reiniciar_labirinto(self):
        ...
        # Limpa os eixos para a próxima execução
        self.ax.clear()
        
        # Configurações iniciais dos eixos, se necessário (ex: título, rótulos, etc.)
        self.ax.set_title("Labirinto 3D")
        self.ax.set_xlabel("Eixo X")
        self.ax.set_ylabel("Eixo Y")
        self.ax.set_zlabel("Eixo Z")

    def guloso(self, xi, yi, zi, xf, yf, zf):
...
if __name__ == '__main__':
    ...
    while True:  # Loop infinito para iniciar novas buscas automaticamente
        lab.reiniciar_labirinto()  # Reinicia o labirinto e variáveis de estado
